...
## Status: Phase 8 (Streamlit UI/UX overhaul) implemented and verified, plus backlog items 7 and 8 shipped on top of it. Phase 9 (React/FastAPI frontend split) is now fully shipped — **9a (FastAPI backend), 9b (minimal frontend), and 9c (full UI) are all implemented and verified**, live and via tests/build. Docs polish stays last at Phase 10.
...
**(9b) Minimal frontend — implemented and verified.** Vite + React + TypeScript scaffold at `frontend/`, sibling to `backend/`. Deliberately unstyled — same "wiring is real, not looks good" bar as 9a — proving the browser/WebSocket/React plumbing works before 9c's polish investment.

- `frontend/src/useTickStream.ts` — a React hook owning the `/ws/ticks` connection. Native `WebSocket` doesn't auto-reconnect, so this hook does: on any drop (including a refused first attempt), it retries with capped exponential backoff (500ms doubling to 8s), badging `"reconnecting"` for the first 5 attempts and `"lost"` after — while continuing to retry in the background the whole time, since a still-down backend eventually comes back with no user action needed. An `unmountedRef` guard stops the retry loop from outliving the component (React 19's dev-only StrictMode double-mount would otherwise leak a phantom connection — confirmed this is exactly what was happening, not a real bug, via a live check: the expected browser-level "WebSocket is closed before the connection is established" warning fires once on mount, harmlessly, and the real connection succeeds right after).
- `frontend/src/ConnectionBadge.tsx` / `TemperatureChart.tsx` (Recharts, the one call made where the design docs didn't specify a library) / `App.tsx` — connection badge, a live sensed-temperature line chart, and a Run/Stop button calling `POST /session/run` end-to-end, all fed by the one hook.
- `frontend/src/types.ts` — hand-kept mirror of `backend/schemas.py`'s `TickRecord` (no codegen step in this minimal scope).
- **Known rough edge, left for 9c:** the tick buffer isn't cleared on reconnect, so after a backend restart the chart's x-axis briefly shows old and new tick numbers concatenated (non-monotonic) until the old data ages out of the 200-tick rolling window. The architecturally correct fix — clear and refetch backlog from `GET /state` on reconnect — is exactly what that endpoint exists for per 9a's plan, but wiring it up is data-hygiene polish, not plumbing-proving, so it's deferred rather than done now.

**Verified live** (Playwright driving a real Chrome instance against the real `uvicorn` + `vite` dev servers, one continuous browser tab throughout so state persists across the sequence — not fresh page loads stitched together):
1. Real backend, real ticks: badge showed "Connected", chart rendered live sensed-temperature data, tick JSON matched the schema.
2. Run/Stop button toggled the real backend's `running` state both directions, confirmed via a follow-up `GET /state`.
3. Full reconnect cycle in one session, same tab, no manual reload: killed the backend process mid-stream — badge went `Connected` → `Reconnecting...` → `Connection lost` (past the 5-attempt threshold) — restarted the backend — badge recovered to `Connected` on its own and new ticks (from the fresh process, tick count restarted from ~0) started flowing again.
4. `npm run build` (`tsc -b && vite build`) succeeds with no type errors — a stricter check than `tsc --noEmit` alone, which caught one real type mismatch in the chart tooltip formatter before this pass closed out.
5. `pytest -q`: 134/134 (untouched — nothing in `backend`/`engine`/`storage` changed this phase).
**(9c) Full UI build — implemented and verified.** The reviewed 12-state mockup, built for real on top of 9a/9b's plumbing. See "Phase 9c: full UI build" below for the details.

## Phase 9c: full UI build

Turns the reviewed mockup (`docs/design-prompts/Phase 9 React Frontend UI.dc.html`) into real code on top of 9a's API and 9b's WebSocket plumbing. No changes to `engine/`'s control or safety logic — same "presentation only" scope Phase 8 had, plus a few small, honest backend surface additions (below) needed because 9a's schema didn't yet carry everything the mockup's content inventory called for.

### Backend additions (small, narrowly scoped — not a re-open of 9a)

Discovered while building the frontend, not planned in advance:
- **`trip_strikes` / `trip_lockout_threshold` added to the per-tick record** (`engine/loop.py`'s `tick()` return dict, `docs/control-loop-architecture.md` §4's schema, `backend/schemas.py`'s `TickRecord`). `app.py` could always read these directly off `loop.interlock` since Streamlit runs in-process; the FastAPI schema never carried them, so the mockup's "Trip strikes" tile had no data source. Test updated: `test_ws_streams_ticks`'s exact-keys assertion.
- **`GET /state` gained a `controls` block** (`backend/schemas.py`'s `ControlsOut`, `SimulationService.get_controls()`) — current mode/setpoint/PID gains/manual heater+override/fault toggles, independent of tick history, via two new public read properties on `Sensor` (`drift_enabled`, `stuck_enabled`). Without this, a freshly-loaded or reconnected frontend had no way to render accurate control values before the first tick landed (the mockup's own Idle state shows a real setpoint, not a placeholder). Test updated: `test_get_state`.
- **`GET /config/scenarios` (new endpoint)** — the named seeded-scenario list (`SimulationService.get_scenarios()`, same name-formatting `app.py`'s `SCENARIOS` dict already used), so the frontend's fault-injection scenario picker isn't guessing at seeds. New test: `test_get_scenarios`.
- **Historian tile shows "not wired," not faked** — the FastAPI backend never constructed a `Historian` at all (Phase 4 only wired one into Streamlit). Rather than fabricate connectivity data, the tile renders the honest current state. Filed as `BACKLOG.md` item 9 (a real scoped gap, not deferred-forever like `POST_MVP_BACKLOG.md`'s items).
- **`backend/main.py` never called `load_dotenv()`** (found post-shipping, while debugging a user report of "my Anthropic key stopped working"). `app.py` has always called it for Streamlit; `backend/main.py` read `os.environ.get("ANTHROPIC_API_KEY")` directly, so AI mode/triage on the FastAPI backend only ever worked if the launching shell already had the key exported — `.env` alone wasn't enough, unlike the Streamlit app. Fixed by adding the same `load_dotenv()` call `app.py` already has. Broke one test's implicit assumption (`test_triage_endpoint_no_client_configured` relied on the ambient environment lacking the key, which is fragile by construction) — fixed by having that test `monkeypatch.delenv` explicitly rather than depend on ambient absence. Verified live: started a fresh `uvicorn` process with `ANTHROPIC_API_KEY` explicitly unset from the shell (only present in `.env`) and got a real, successful Claude response from `POST /triage` — confirms `.env`-only setups now work. Suite: 135/135.

Full pytest suite: **135/135** (1 new test, 2 updated for the schema additions — `engine`/`storage`/`app.py` logic itself untouched).

### Frontend build (`frontend/`)

- **Tooling**: Tailwind v4 (`@tailwindcss/vite`, no separate PostCSS config needed) + shadcn/ui (`radix` base, `nova` preset) — the CLI copies component source into `frontend/src/components/ui/`, not a black-box import. `@/` path alias wired through both `vite.config.ts` and `tsconfig.app.json`. Fixed dark control-room theme (`class="dark"` hardcoded on `<html>`, no light-mode variant) — a deliberate single-look choice per the design brief, not something that should follow OS theme.
- **`src/lib/severity.ts`** — the TS mirror of `app.py`'s Phase 8 GREEN/AMBER/RED/GRAY module constants and derivation logic (`globalStatus()`, `decisionLogRowSeverity()`, per-tile severity helpers), same OKLCH values, so the two frontends never drift on what counts as "critical." Connection state (`CONNECTION_*` exports) is deliberately a *separate* type from `Severity` — violet, never red/amber, so a dropped WebSocket can never read as a safety trip. This was caught directly from the mockup's own script constants (`CONN_LOST` uses violet, not red) rather than assumed.
- **`src/hooks/useSimulationState.ts`** — the app's one state store: WebSocket connection, tick buffer, and every control value, plus the actions that mutate them (optimistic local update + fire-and-forget POST, safe at this project's single-operator/single-client scope). Fixes 9b's known rough edge here: `resyncFromServer()` calls `GET /state` on *every* connect and reconnect and replaces the tick buffer wholesale, rather than only ever appending — a reconnect after a backend restart now cleanly shows "no data yet" instead of concatenating stale and fresh tick numbers. `appendTick()` also drops any tick not strictly newer than the buffer's last one, guarding against a live tick racing ahead of an in-flight resync.
- **`src/hooks/useEventToasts.ts`** — fires a toast on a genuine false→true transition between two *consecutive* live ticks (lockout, hard trip, override, AI fallback, stuck/spike fault). Deliberately does not replay anything from a `GET /state` backlog fetch — a resync jumps the tick number non-consecutively, which this hook treats as "rebaseline silently, don't toast." This was a real design question worked through with the user before building (see `POST_MVP_BACKLOG.md` item 1): toasts are a "notice this now" affordance, not a notification inbox: the decision log (already durable via the resync fix above) is the record of anything that happened while disconnected, and the status pill surfaces anything still-active on reconnect.
- **Components** (`src/components/`): `Toolbar` (connection badge + status pill + Run/Reset/Reset-Interlock), `TwoStageButton` (generic confirm-on-second-click, used by both Reset actions — no modal, per the design brief's explicit call for speed over friction in a live demo tool), `ControllerCard`, `FaultInjectionCard`, `TelemetryTiles` (7-tile row), `TemperatureChart` (kept Recharts from 9b rather than hand-rolling the mockup's raw SVG — already validated, just restyled to the mockup's true/sensed/setpoint colors and added the true+setpoint lines 9b's minimal version didn't have), `AiReasoningPanel` (one fixed DOM slot in every mode, empty placeholder vs. filled content swapped via conditional render — never mounted/unmounted, so mode switches can't reflow neighbors), `DecisionLog` (severity-tinted rows, 50-row default + full-history toggle, matching `app.py`'s own defaults), `TriagePanel` (see below).
- **Decision log row animation**: a new row eases in from a brief white flash to its resting color via CSS `@starting-style` + `transition` (`index.css`) — fires once per newly-mounted row (by React key = tick number) and never replays for rows already on screen, no JS timer/cleanup needed.
- **Triage with Claude, kept even though absent from the Phase 9 design brief's content inventory** — an apparent oversight in the brief (Phase 7 shipped this before the brief was written, and the brief claims Phase 8 parity but doesn't list it). Rather than silently drop a shipped feature, `TriagePanel.tsx` reimplements the same combined trigger condition as backlog item 7 (`app.py`): enabled once a detector flag is active or a recent interlock result was non-`allow`.

### Verified two ways

1. **`pytest -q`: 135/135.** **`npm run build`** (`tsc -b && vite build`): clean, no type errors. **`npm run lint`** (oxlint): clean except three pre-existing warnings inside shadcn-generated files under `components/ui/` (not project code).
2. **Live** (real `uvicorn` + `vite` dev servers, one continuous browser tab via Claude in Chrome, matching the project's established Playwright-style verification pattern): Idle state matches the mockup's placeholder values; Manual mode driven to slew-limit clamping then a real hard trip (self-clearing exactly as documented in Phase 6's verification, trip strikes 1/2, no lockout) with the decision log's amber→red→amber color sequence rendering correctly and a "Hard trip fired" toast firing; PID and AI mode switches confirmed the controller card holds a stable height (no reflow) and the AI reasoning panel correctly showed a "no Anthropic client configured" error state (no API key in this environment) with the same wording `AIController` already produces; stuck-at fault toggle froze the chart's sensed line, reddened the Active Faults tile, and fired a "Stuck-at fault injected" toast; both `Reset` and `Reset Interlock` two-stage buttons confirmed correctly (first click arms with a red "Confirm...?" label, second click within the window commits — a same-batch double-click test caught that a single click alone does nothing, matching the design intent); killing and restarting the backend process mid-session showed the connection badge cycle to violet "Connection lost," a diagonal-stripe stale overlay on both the chart and decision log with a live "last update Ns ago" counter, then a clean automatic recovery to "Live" with the tick buffer correctly starting fresh at the new process's tick 1 rather than concatenating with the old session's ticks — direct confirmation of the reconnect-buffer fix. Zero unexpected browser console errors (only the one expected React StrictMode phantom-connection warning already documented in 9b).

## Phase 9c's UI design (mockup complete, implementation shipped above)

Covers 9c specifically (see the sub-phase breakdown above for 9a/9b). User-requested phase beyond the original architecture doc's scope, same status as Phase 8's design was before it got built. `docs/design-prompts/phase-9-react-frontend-prompt.md` / `Phase 9 React Frontend UI.dc.html`. **Design work is genuinely done** — reviewed directly, not taken on faith: a full interactive HTML mockup (~44KB), not a stub. The unconstrained version of Phase 8's problem, once the backend splits into a FastAPI service streaming ticks over a WebSocket. Shares Phase 8's content inventory and severity color system (green=nominal/allowed, amber=degraded, red=critical, gray=inactive), but uses freedoms Streamlit doesn't have: sticky panels, toasts for one-shot events (spike fired, lockout engaged), animated state transitions, and a live WebSocket connection badge (connected/reconnecting/lost), since silently-stale data is a real failure mode worth surfacing on its own. Deliberately distinguishes *system-imposed* critical states (lockout, hard trip, AI fallback) from the *operator-chosen* one (manual override) — same severity tier, visually distinct, since the operator caused one of these on purpose. **Delivered: all 12 requested states** (Idle, Manual/PID/AI nominal, AI degraded, AI fallback, sensor fault, reject/clamp, hard trip, locked out, manual override, plus "connection lost"), plus a written implied-primitives legend citing shadcn/ui + Tailwind + Radix for each nonstandard interaction (e.g. toasts via shadcn Sonner, Reset/Reset Interlock as a two-stage button rather than a modal). Implementation shipped — see "Phase 9c: full UI build" above for what was actually built and how it was verified.
...
Bigger-picture ideas that only make sense if the project's scope grew beyond a local single-operator demo tool (raised in conversation, deliberately not scheduled) live in `POST_MVP_BACKLOG.md` instead — separate from `BACKLOG.md` since these aren't refinements of the current scope, they're "would reconsider if the scope itself changed."
